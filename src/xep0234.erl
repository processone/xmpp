...
pp(jingle_ft_file, 8) ->
    [date, desc, hash, 'hash-used', 'media-type', name,
     size, range];
...
records() ->
    [{text, 2}, {jingle_ft_range, 3}, {jingle_ft_file, 8},
     {jingle_ft_description, 1}, {jingle_ft_received, 2},
     {jingle_ft_checksum, 3}, {jingle_ft_error, 1}].
...
decode_jingle_ft_checksum(__TopXMLNS, __Opts,
    ...
    File = decode_jingle_ft_checksum_els(__TopXMLNS, __Opts,
					 _els, error),
    {Creator, Name} =
	decode_jingle_ft_checksum_attrs(__TopXMLNS, _attrs,
					undefined, undefined),
...
decode_jingle_ft_checksum_els(__TopXMLNS, __Opts,
	...
	of
      <<"urn:xmpp:jingle:apps:file-transfer:5">> ->
	  decode_jingle_ft_checksum_els(__TopXMLNS, __Opts, _els,
					{value,
					 decode_jingle_ft_file(<<"urn:xmpp:jingle:apps:file-transfer:5">>,
							       __Opts, _el)});
      _ ->
...
encode_jingle_ft_checksum({jingle_ft_checksum, Creator,
			   Name, File},
			  __TopXMLNS) ->
    __NewTopXMLNS =
...
decode_jingle_ft_checksum_attr_creator(__TopXMLNS,
    ...
    case catch dec_enum(_val, [initiator, responder]) of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
			{bad_attr_value, <<"creator">>, <<"checksum">>,
			 __TopXMLNS}});
      _res -> _res
    end.
...
decode_jingle_ft_received(__TopXMLNS, __Opts,
    ...
    {Creator, Name} =
	decode_jingle_ft_received_attrs(__TopXMLNS, _attrs,
					undefined, undefined),
...
encode_jingle_ft_received({jingle_ft_received, Creator,
			   Name},
			  __TopXMLNS) ->
    __NewTopXMLNS =
	xmpp_codec:choose_top_xmlns(<<"urn:xmpp:jingle:apps:file-transfer:5">>,
...
decode_jingle_ft_received_attr_creator(__TopXMLNS,
    ...
    case catch dec_enum(_val, [initiator, responder]) of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
			{bad_attr_value, <<"creator">>, <<"received">>,
			 __TopXMLNS}});
      _res -> _res
    end.
...
decode_jingle_ft_file(__TopXMLNS, __Opts,
    ...
    {Desc, Hash_used, Range, Size, Name, Media_type, Date,
     Hash} =
	decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els, [],
				  undefined, undefined, undefined, undefined,
				  undefined, undefined, []),
    {jingle_ft_file, Date, Desc, Hash, Hash_used,
     Media_type, Name, Size, Range}.

decode_jingle_ft_file_els(__TopXMLNS, __Opts, [], Desc,
			  Hash_used, Range, Size, Name, Media_type, Date,
			  Hash) ->
    {lists:reverse(Desc), Hash_used, Range, Size, Name,
     Media_type, Date, lists:reverse(Hash)};
decode_jingle_ft_file_els(__TopXMLNS, __Opts,
	...
	of
      <<"urn:xmpp:jingle:apps:file-transfer:5">> ->
	  decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els,
				    Desc, Hash_used, Range, Size, Name,
				    Media_type,
				    decode_jingle_ft_date(<<"urn:xmpp:jingle:apps:file-transfer:5">>,
							  __Opts, _el),
				    Hash);
	...
	of
	  ...
	  decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els,
				    [decode_jingle_ft_desc(<<"urn:xmpp:jingle:apps:file-transfer:5">>,
							   __Opts, _el)
				     | Desc],
				    Hash_used, Range, Size, Name, Media_type,
				    Date, Hash);
      _ ->
	  decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els,
				    Desc, Hash_used, Range, Size, Name,
	...
	of
	  ...
	  decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els,
				    ...
				    xep0300:decode_hash_used(<<"urn:xmpp:hashes:2">>,
							     __Opts, _el),
				    Range, Size, Name, Media_type, Date, Hash);
      _ ->
	  decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els,
				    Desc, Hash_used, Range, Size, Name,
				    Media_type, Date, Hash)
...
decode_jingle_ft_file_els(__TopXMLNS, __Opts,
	...
	of
      <<"urn:xmpp:jingle:apps:file-transfer:5">> ->
	  decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els,
				    Desc, Hash_used, Range,
				    decode_jingle_ft_size(<<"urn:xmpp:jingle:apps:file-transfer:5">>,
							  __Opts, _el),
				    Name, Media_type, Date, Hash);
      _ ->
	  decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els,
				    Desc, Hash_used, Range, Size, Name,
				    Media_type, Date, Hash)
...
decode_jingle_ft_file_els(__TopXMLNS, __Opts,
	...
	of
	  ...
	  decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els,
				    ...
				    decode_jingle_ft_range(<<"urn:xmpp:jingle:apps:file-transfer:5">>,
							   __Opts, _el),
				    Size, Name, Media_type, Date, Hash);
      _ ->
	  decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els,
				    Desc, Hash_used, Range, Size, Name,
				    Media_type, Date, Hash)
...
decode_jingle_ft_file_els(__TopXMLNS, __Opts,
			  ...
			  Media_type, Date, Hash) ->
    decode_jingle_ft_file_els(__TopXMLNS, __Opts, _els,
			      Desc, Hash_used, Range, Size, Name, Media_type,
			      Date, Hash).

encode_jingle_ft_file({jingle_ft_file, Date, Desc, Hash,
		       Hash_used, Media_type, Name, Size, Range},
		      __TopXMLNS) ->
    __NewTopXMLNS =
	xmpp_codec:choose_top_xmlns(<<"urn:xmpp:jingle:apps:file-transfer:5">>,
				    [], __TopXMLNS),
...
'encode_jingle_ft_file_$desc'([Desc | _els], __TopXMLNS,
			      _acc) ->
    'encode_jingle_ft_file_$desc'(_els, __TopXMLNS,
...
'encode_jingle_ft_file_$hash'([Hash | _els], __TopXMLNS,
			      _acc) ->
    'encode_jingle_ft_file_$hash'(_els, __TopXMLNS,
				  [xep0300:encode_hash(Hash, __TopXMLNS)
				   | _acc]).

decode_jingle_ft_range(__TopXMLNS, __Opts,
		       {xmlel, <<"range">>, _attrs, _els}) ->
    Hash = decode_jingle_ft_range_els(__TopXMLNS, __Opts,
				      _els, []),
    {Offset, Length} =
	decode_jingle_ft_range_attrs(__TopXMLNS, _attrs,
				     undefined, undefined),
...
decode_jingle_ft_range_els(__TopXMLNS, __Opts,
	...
	of
      <<"urn:xmpp:hashes:2">> ->
	  decode_jingle_ft_range_els(__TopXMLNS, __Opts, _els,
				     [xep0300:decode_hash(<<"urn:xmpp:hashes:2">>,
							  __Opts, _el)
				      | Hash]);
      _ ->
...
decode_jingle_ft_range_attrs(__TopXMLNS,
			     ...
			     Length) ->
    decode_jingle_ft_range_attrs(__TopXMLNS, _attrs, _val,
				 Length);
decode_jingle_ft_range_attrs(__TopXMLNS,
			     [{<<"length">>, _val} | _attrs], Offset,
			     _Length) ->
    decode_jingle_ft_range_attrs(__TopXMLNS, _attrs, Offset,
				 _val);
...
encode_jingle_ft_range({jingle_ft_range, Offset, Length,
			Hash},
		       __TopXMLNS) ->
    __NewTopXMLNS =
	xmpp_codec:choose_top_xmlns(<<"urn:xmpp:jingle:apps:file-transfer:5">>,
...
'encode_jingle_ft_range_$hash'([Hash | _els],
			       __TopXMLNS, _acc) ->
    'encode_jingle_ft_range_$hash'(_els, __TopXMLNS,
				   [xep0300:encode_hash(Hash, __TopXMLNS)
				    | _acc]).
...
decode_jingle_ft_size(__TopXMLNS, __Opts,
    ...
    Cdata = decode_jingle_ft_size_els(__TopXMLNS, __Opts,
				      _els, <<>>),
    Cdata.
...
decode_jingle_ft_size_els(__TopXMLNS, __Opts,
    ...
    decode_jingle_ft_size_els(__TopXMLNS, __Opts, _els,
...
decode_jingle_ft_name(__TopXMLNS, __Opts,
    ...
    Cdata = decode_jingle_ft_name_els(__TopXMLNS, __Opts,
				      _els, <<>>),
    Cdata.
...
decode_jingle_ft_name_els(__TopXMLNS, __Opts,
			  [{xmlcdata, _data} | _els], Cdata) ->
    decode_jingle_ft_name_els(__TopXMLNS, __Opts, _els,
			      <<Cdata/binary, _data/binary>>);
decode_jingle_ft_name_els(__TopXMLNS, __Opts,
			  [_ | _els], Cdata) ->
    decode_jingle_ft_name_els(__TopXMLNS, __Opts, _els,
...
decode_jingle_ft_desc(__TopXMLNS, __Opts,
    ...
    Data = decode_jingle_ft_desc_els(__TopXMLNS, __Opts,
				     _els, <<>>),
    Lang = decode_jingle_ft_desc_attrs(__TopXMLNS, _attrs,
				       undefined),
    {text, Lang, Data}.
...
decode_jingle_ft_desc_els(__TopXMLNS, __Opts,
    ...
    decode_jingle_ft_desc_els(__TopXMLNS, __Opts, _els,
...
decode_jingle_ft_date(__TopXMLNS, __Opts,
    ...
    Cdata = decode_jingle_ft_date_els(__TopXMLNS, __Opts,
				      _els, <<>>),
    Cdata.
...
decode_jingle_ft_date_els(__TopXMLNS, __Opts,
			  [{xmlcdata, _data} | _els], Cdata) ->
    decode_jingle_ft_date_els(__TopXMLNS, __Opts, _els,
			      <<Cdata/binary, _data/binary>>);
decode_jingle_ft_date_els(__TopXMLNS, __Opts,
			  [_ | _els], Cdata) ->
    decode_jingle_ft_date_els(__TopXMLNS, __Opts, _els,
