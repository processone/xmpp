...
-spec encode(form(), binary(),
	     [deliver | digest | digest_frequency | expire |
	      include_body | 'show-values' | subscription_type |
...
encode(List, Lang, Required) ->
    Fs = [case Opt of
	    {deliver, Val} ->
		[encode_deliver(Val, Lang,
				lists:member(deliver, Required))];
	    {digest, Val} ->
		[encode_digest(Val, Lang,
		...
		[encode_digest_frequency(Val, Lang,
					 lists:member(digest_frequency,
						      Required))];
	    {expire, Val} ->
		[encode_expire(Val, Lang,
			       lists:member(expire, Required))];
	    {include_body, Val} ->
		[encode_include_body(Val, Lang,
