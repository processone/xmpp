...
decode_presence_show_cdata(__TopXMLNS, _val) ->
    try dec_enum(_val, [away, chat, dnd, xa]) of
        _res -> _res
    catch
        error:_ ->
            erlang:error({xmpp_codec,
...
decode_iq_attr_type(__TopXMLNS, _val) ->
    try dec_enum(_val, [get, set, result, error]) of
        _res -> _res
    catch
        error:_ ->
            erlang:error({xmpp_codec,
