...
do_get_name({service, _, _, _, _, _, _, _, _, _, _,
	     _}) ->
    <<"service">>;
...
pp(service, 11) ->
    [action, expires, host, name, password, port,
     restricted, transport, type, username, xdata];
...
decode_credentials(__TopXMLNS, __Opts,
    ...
    Services = decode_credentials_els(__TopXMLNS, __Opts,
				      _els, []),
...
decode_credentials_els(__TopXMLNS, __Opts,
	...
	of
      <<"urn:xmpp:extdisco:2">> ->
	  decode_credentials_els(__TopXMLNS, __Opts, _els,
...
'encode_credentials_$services'([Services | _els],
			       __TopXMLNS, _acc) ->
    'encode_credentials_$services'(_els, __TopXMLNS,
				   [encode_service(Services, __TopXMLNS)
				    | _acc]).

decode_services(__TopXMLNS, __Opts,
		{xmlel, <<"services">>, _attrs, _els}) ->
    List = decode_services_els(__TopXMLNS, __Opts, _els,
			       []),
    Type = decode_services_attrs(__TopXMLNS, _attrs,
				 undefined),
    {services, Type, List}.
...
decode_services_els(__TopXMLNS, __Opts,
	...
	of
      <<"urn:xmpp:extdisco:2">> ->
	  decode_services_els(__TopXMLNS, __Opts, _els,
			      [decode_service(<<"urn:xmpp:extdisco:2">>, __Opts,
					      _el)
			       | List]);
...
'encode_services_$list'([List | _els], __TopXMLNS,
			_acc) ->
    'encode_services_$list'(_els, __TopXMLNS,
...
decode_service(__TopXMLNS, __Opts,
    ...
    Xdata = decode_service_els(__TopXMLNS, __Opts, _els,
			       undefined),
    {Action, Expires, Host, Name, Password, Port,
     Restricted, Transport, Type, Username} =
	decode_service_attrs(__TopXMLNS, _attrs, undefined,
			     undefined, undefined, undefined, undefined,
			     undefined, undefined, undefined, undefined,
			     undefined),
    {service, Action, Expires, Host, Name, Password, Port,
     Restricted, Transport, Type, Username, Xdata}.
...
decode_service_els(__TopXMLNS, __Opts,
	...
	of
      <<"jabber:x:data">> ->
	  decode_service_els(__TopXMLNS, __Opts, _els,
...
decode_service_attrs(__TopXMLNS,
		     ...
		     Username) ->
    decode_service_attrs(__TopXMLNS, _attrs, _val, Expires,
			 Host, Name, Password, Port, Restricted, Transport,
			 Type, Username);
decode_service_attrs(__TopXMLNS,
		     [{<<"expires">>, _val} | _attrs], Action, _Expires,
		     Host, Name, Password, Port, Restricted, Transport, Type,
		     Username) ->
    decode_service_attrs(__TopXMLNS, _attrs, Action, _val,
			 Host, Name, Password, Port, Restricted, Transport,
			 Type, Username);
decode_service_attrs(__TopXMLNS,
		     ...
		     Name, Password, Port, Restricted, Transport, Type,
		     Username) ->
    decode_service_attrs(__TopXMLNS, _attrs, Action,
			 Expires, _val, Name, Password, Port, Restricted,
...
decode_service_attrs(__TopXMLNS,
		     ...
		     Username) ->
    decode_service_attrs(__TopXMLNS, _attrs, Action,
			 Expires, Host, _val, Password, Port, Restricted,
			 Transport, Type, Username);
decode_service_attrs(__TopXMLNS,
		     ...
		     Type, Username) ->
    decode_service_attrs(__TopXMLNS, _attrs, Action,
			 Expires, Host, Name, _val, Port, Restricted, Transport,
			 Type, Username);
decode_service_attrs(__TopXMLNS,
		     ...
		     Username) ->
    decode_service_attrs(__TopXMLNS, _attrs, Action,
			 Expires, Host, Name, Password, _val, Restricted,
			 Transport, Type, Username);
decode_service_attrs(__TopXMLNS,
		     ...
		     Type, Username) ->
    decode_service_attrs(__TopXMLNS, _attrs, Action,
			 Expires, Host, Name, Password, Port, _val, Transport,
			 Type, Username);
decode_service_attrs(__TopXMLNS,
		     ...
		     Type, Username) ->
    decode_service_attrs(__TopXMLNS, _attrs, Action,
			 Expires, Host, Name, Password, Port, Restricted, _val,
			 Type, Username);
decode_service_attrs(__TopXMLNS,
...
encode_service({service, Action, Expires, Host, Name,
		Password, Port, Restricted, Transport, Type, Username,
		Xdata},
	       __TopXMLNS) ->
    __NewTopXMLNS =
	xmpp_codec:choose_top_xmlns(<<"urn:xmpp:extdisco:2">>,
...
decode_service_attr_expires(__TopXMLNS, _val) ->
    case catch dec_utc(_val) of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
			{bad_attr_value, <<"expires">>, <<"service">>,
			 __TopXMLNS}});
      _res -> _res
    end.
...
decode_service_attr_restricted(__TopXMLNS, _val) ->
    case catch dec_bool(_val) of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
			{bad_attr_value, <<"restricted">>, <<"service">>,
			 __TopXMLNS}});
      _res -> _res
    end.
...
decode_service_attr_transport(__TopXMLNS, _val) ->
    case catch dec_enum(_val, [tcp, udp]) of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
			{bad_attr_value, <<"transport">>, <<"service">>,
			 __TopXMLNS}});
      _res -> _res
    end.
