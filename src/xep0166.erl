...
do_decode(<<"content">>, <<"urn:xmpp:jingle:1">>, El,
	  Opts) ->
    decode_jingle_content(<<"urn:xmpp:jingle:1">>, Opts,
			  El);
...
do_decode(<<"text">>, <<"urn:xmpp:jingle:1">>, El,
	  Opts) ->
    decode_jingle_reason_text(<<"urn:xmpp:jingle:1">>, Opts,
			      El);
...
do_decode(<<"gone">>, <<"urn:xmpp:jingle:1">>, El,
	  Opts) ->
    decode_jingle_reason_gone(<<"urn:xmpp:jingle:1">>, Opts,
			      El);
...
do_decode(<<"busy">>, <<"urn:xmpp:jingle:1">>, El,
	  Opts) ->
    decode_jingle_reason_busy(<<"urn:xmpp:jingle:1">>, Opts,
			      El);
...
get_els({jingle_content, _creator, _disposition, _name,
	 _senders, _sub_els}) ->
    _sub_els;
get_els({jingle, _action, _sid, _initiator, _responder,
...
set_els({jingle_content, _creator, _disposition, _name,
	 _senders, _},
	_sub_els) ->
    {jingle_content, _creator, _disposition, _name,
     _senders, _sub_els};
set_els({jingle, _action, _sid, _initiator, _responder,
	 _content, _reason, _},
	_sub_els) ->
    {jingle, _action, _sid, _initiator, _responder,
     _content, _reason, _sub_els}.
...
pp(jingle, 7) ->
    [action, sid, initiator, responder, content, reason,
     sub_els];
pp(_, _) -> no.
...
decode_jingle(__TopXMLNS, __Opts,
	...
	decode_jingle_attrs(__TopXMLNS, _attrs, undefined,
			    undefined, undefined, undefined),
    {jingle, Action, Sid, Initiator, Responder, Content,
     Reason, __Els}.
...
decode_jingle_els(__TopXMLNS, __Opts,
	...
	of
      <<"urn:xmpp:jingle:1">> ->
	  decode_jingle_els(__TopXMLNS, __Opts, _els,
			    [decode_jingle_content(<<"urn:xmpp:jingle:1">>,
						   __Opts, _el)
			     | Content],
			    Reason, __Els);
      _ ->
	  decode_jingle_els(__TopXMLNS, __Opts, _els, Content,
			    Reason, [_el | __Els])
    end;
decode_jingle_els(__TopXMLNS, __Opts,
...
decode_jingle_attrs(__TopXMLNS,
		    ...
		    Initiator, Responder) ->
    decode_jingle_attrs(__TopXMLNS, _attrs, _val, Sid,
			Initiator, Responder);
decode_jingle_attrs(__TopXMLNS,
		    [{<<"sid">>, _val} | _attrs], Action, _Sid, Initiator,
		    Responder) ->
    decode_jingle_attrs(__TopXMLNS, _attrs, Action, _val,
			Initiator, Responder);
decode_jingle_attrs(__TopXMLNS,
		    [{<<"initiator">>, _val} | _attrs], Action, Sid,
		    _Initiator, Responder) ->
    decode_jingle_attrs(__TopXMLNS, _attrs, Action, Sid,
			_val, Responder);
decode_jingle_attrs(__TopXMLNS,
...
encode_jingle({jingle, Action, Sid, Initiator,
	       Responder, Content, Reason, __Els},
	      __TopXMLNS) ->
    __NewTopXMLNS =
...
'encode_jingle_$content'([Content | _els], __TopXMLNS,
			 _acc) ->
    'encode_jingle_$content'(_els, __TopXMLNS,
...
decode_jingle_attr_initiator(__TopXMLNS, _val) ->
    case catch jid:decode(_val) of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
			{bad_attr_value, <<"initiator">>, <<"jingle">>,
			 __TopXMLNS}});
      _res -> _res
    end.
...
decode_jingle_attr_responder(__TopXMLNS, _val) ->
    case catch jid:decode(_val) of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
			{bad_attr_value, <<"responder">>, <<"jingle">>,
			 __TopXMLNS}});
      _res -> _res
    end.
...
decode_jingle_content(__TopXMLNS, __Opts,
    ...
    __Els = decode_jingle_content_els(__TopXMLNS, __Opts,
				      _els, []),
    {Creator, Disposition, Name, Senders} =
	decode_jingle_content_attrs(__TopXMLNS, _attrs,
				    undefined, undefined, undefined, undefined),
    {jingle_content, Creator, Disposition, Name, Senders,
     __Els}.
...
decode_jingle_content_els(__TopXMLNS, __Opts,
    ...
    case proplists:get_bool(ignore_els, __Opts) of
      true ->
	  decode_jingle_content_els(__TopXMLNS, __Opts, _els,
				    [_el | __Els]);
      false ->
	  __XMLNS = xmpp_codec:get_attr(<<"xmlns">>, _attrs,
					__TopXMLNS),
...
decode_jingle_content_attrs(__TopXMLNS,
    ...
    decode_jingle_content_attrs(__TopXMLNS, _attrs, _val,
				Disposition, Name, Senders);
decode_jingle_content_attrs(__TopXMLNS,
			    [{<<"disposition">>, _val} | _attrs], Creator,
			    _Disposition, Name, Senders) ->
    decode_jingle_content_attrs(__TopXMLNS, _attrs, Creator,
				_val, Name, Senders);
decode_jingle_content_attrs(__TopXMLNS,
			    [{<<"name">>, _val} | _attrs], Creator, Disposition,
			    _Name, Senders) ->
    decode_jingle_content_attrs(__TopXMLNS, _attrs, Creator,
				Disposition, _val, Senders);
decode_jingle_content_attrs(__TopXMLNS,
			    [{<<"senders">>, _val} | _attrs], Creator,
			    Disposition, Name, _Senders) ->
    decode_jingle_content_attrs(__TopXMLNS, _attrs, Creator,
				Disposition, Name, _val);
...
encode_jingle_content({jingle_content, Creator,
		       Disposition, Name, Senders, __Els},
		      __TopXMLNS) ->
    __NewTopXMLNS =
	xmpp_codec:choose_top_xmlns(<<"urn:xmpp:jingle:1">>, [],
				    __TopXMLNS),
...
decode_jingle_content_attr_senders(__TopXMLNS, _val) ->
	...
	of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
			{bad_attr_value, <<"senders">>, <<"content">>,
			 __TopXMLNS}});
      _res -> _res
    end.
...
'encode_jingle_reason_$text'([Text | _els], __TopXMLNS,
			     _acc) ->
    'encode_jingle_reason_$text'(_els, __TopXMLNS,
...
decode_jingle_reason_text(__TopXMLNS, __Opts,
    ...
    Data = decode_jingle_reason_text_els(__TopXMLNS, __Opts,
					 _els, <<>>),
...
decode_jingle_reason_text_els(__TopXMLNS, __Opts,
    ...
    decode_jingle_reason_text_els(__TopXMLNS, __Opts, _els,
...
decode_jingle_reason_text_attrs(__TopXMLNS,
    ...
    decode_jingle_reason_text_attrs(__TopXMLNS, _attrs,
				    _val);
decode_jingle_reason_text_attrs(__TopXMLNS,
				[_ | _attrs], Lang) ->
    decode_jingle_reason_text_attrs(__TopXMLNS, _attrs,
...
decode_jingle_error_security_required(__TopXMLNS,
				      __Opts,
				      {xmlel, <<"security-required">>, _attrs,
				       _els}) ->
